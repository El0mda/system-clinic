  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-text-primary">Patients</h1>
          <p className="text-sm text-text-secondary mt-1">{patients.length} total patients</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <Button variant="outline" size="md" onClick={() => setShowFilters(!showFilters)}>
            ...
            Filters
            {filterBadge && <span className="ml-1 w-2 h-2 rounded-full bg-primary-500" />}
          </Button>
          {showFilters && (
            <div className="absolute right-0 top-full mt-2 z-50 w-64 rounded-xl border border-border/40 bg-surface shadow-xl p-4 space-y-4">
              <div>
                <p className="text-xs font-medium text-text-tertiary mb-2">Gender</p>
                <div className="flex gap-1.5">
                  {(["All", "Male", "Female"] as GenderFilter[]).map((g) => (
                    <button key={g} onClick={() => setGenderFilter(g)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${genderFilter === g ? "bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-300" : "text-text-secondary hover:text-text-primary hover:bg-surface-secondary"}`}>{g}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-1.5">
                  {(["All", "Active", "Inactive", "New"] as StatusFilter[]).map((s) => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === s ? "bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-300" : "text-text-secondary hover:text-text-primary hover:bg-surface-secondary"}`}>{s}</button>
                  ))}
                </div>
              </div>
              {(genderFilter !== "All" || statusFilter !== "All") && (
                <button onClick={() => { setGenderFilter("All"); setStatusFilter("All"); }}
                  className="text-xs font-medium text-primary-600 hover:text-primary-700">Clear filters</button>
              )}
            </div>
          )}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/30 bg-surface-secondary/50 dark:bg-surface-tertiary/30">
                ...
                <th className="text-left text-xs font-semibold text-text-tertiary uppercase tracking-wider px-4 py-3">Contact</th>
                <th className="text-left text-xs font-semibold text-text-tertiary uppercase tracking-wider px-4 py-3">Gender</th>
                <th className="text-left text-xs font-semibold text-text-tertiary uppercase tracking-wider px-4 py-3">Age</th>
                <th className="text-left text-xs font-semibold text-text-tertiary uppercase tracking-wider px-4 py-3">Last Visit</th>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-border/20 last:border-0 hover:bg-surface-secondary/50 dark:hover:bg-surface-tertiary/20 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-sm text-text-secondary">{p.gender}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Patient">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button variant="primary" size="md" onClick={handleAdd} disabled={!form.name || !form.email}>Add Patient</Button>
          </div>
        </div>
      </Modal>
    </div>
